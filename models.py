...
def create_cnn_model(fingerprint_input, model_settings, model_size_info, is_training):
  ...
  # first conv
  first_weights = tf.Variable(tf.truncated_normal([first_filter_height, first_filter_width, 1, first_filter_count], stddev=0.01))
  first_bias = tf.Variable(tf.zeros([first_filter_count]))
  first_conv = tf.nn.conv2d(fingerprint_4d, first_weights, [1, first_filter_stride_y, first_filter_stride_x, 1], 'VALID') + first_bias
  first_conv = tf.layers.batch_normalization(first_conv, training=is_training, name='bn1')
  first_relu = tf.nn.relu(first_conv)
  if is_training:
    first_dropout = tf.nn.dropout(first_relu, dropout_prob)
  else:
  ...
  if is_training:
    ...
    second_dropout = second_relu
  second_conv_output_width = math.ceil((first_conv_output_width - second_filter_width + 1) / second_filter_stride_x)
  second_conv_output_height = math.ceil((first_conv_output_height - second_filter_height + 1) / second_filter_stride_y)
  ...
  # linear layer
  W = tf.get_variable('W', shape=[second_conv_element_count, linear_layer_size], initializer=tf.contrib.layers.xavier_initializer())
...
def create_crnn_model(fingerprint_input, model_settings, model_size_info, is_training):
  ...
  first_weights = tf.get_variable('W', shape=[first_filter_height, first_filter_width, 1, first_filter_count], initializer=tf.contrib.layers.xavier_initializer())
  ...
  if is_training:
    ...
    first_dropout = first_relu
  first_conv_output_width = int(math.floor((input_frequency_size - first_filter_width + first_filter_stride_x) / first_filter_stride_x))
  first_conv_output_height = int(math.floor((input_time_size - first_filter_height + first_filter_stride_y) / first_filter_stride_y))
  ...
  if layer_norm:
    ...
    for i in range(num_rnn_layers):
      cell_fw.append(tf.contrib.rnn.GRUCell(RNN_units)) # tf.contrib.rnn.LSTMCell tf.contrib.rnn.GRUCell
      if bidirectional:
        cell_bw.append(tf.contrib.rnn.GRUCell(RNN_units))   # tf.contrib.rnn.LSTMCell tf.contrib.rnn.GRUCell

  if bidirectional:
    outputs, output_state_fw, output_state_bw = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(cell_fw, cell_bw, flow, dtype=tf.float32)
  ...
  first_fc_weights = tf.get_variable('fcw', shape=[flow_dim, first_fc_output_channels], initializer=tf.contrib.layers.xavier_initializer())
  ...
  final_fc_weights = tf.Variable(tf.truncated_normal([first_fc_output_channels, label_count], stddev=0.01))
